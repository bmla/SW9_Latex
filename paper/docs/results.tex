...
The success rate's mean and standard deviation, M(St.D.) for each technique for small(S) and large(L) target sizes can be seen in \Cref{tab:meanHitTechnique} and in \Cref{fig:successResults} the results are shown as a graph.
...
\Cref{tab:meanTimesTechnique} shows the mean time per target in seconds for each of the techniques as well as their standard deviation for both target sizes. 
\begin{table}[H]
	\centering
...
For the the small target, we got $(p<0.001)$, $(F(2.740, 1304.290)=26.523)$, (Greenhouse-Geisser correction: 0.913).
