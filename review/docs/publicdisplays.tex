...
%gut,
%syncing
To support animated content, the display saves 20 screenshots per second% NO between 10 and 20updates on features were enough, but never mentions a specific number, nor the size of the   queue.
 in a queue with timestamps. When connected to a phone, it will syncronize its clock so it can compare the timestamp from the phone with the queue and select the appropiate screenshot for "feature point comparison". %"feature comparison. rewrite. 
...
Jacucci et al. adopted touch as a base for their ``Worlds of Information'' system.  % bad
